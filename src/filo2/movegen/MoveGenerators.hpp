...
#include "filo2/base/BinaryHeap.hpp"
#include "filo2/base/Flat2DVector.hpp"
#include "filo2/base/SparseIntSet.hpp"
#include "filo2/base/VectorView.hpp"
...
namespace cobra {
    ...
    public:
        ...
        inline friend std::ostream& operator<<(std::ostream& stream, const MoveGenerator& move) {
            stream << "(" << move.i << "," << move.j << ", )";
            return stream;
        }
    ...
    public:
        MoveGenerators(const Instance& instance, int k)
            ...
            const int ineighbors_begin = 1;

            for (int i = instance.get_vertices_begin(); i < instance.get_vertices_end(); ++i) {
                const auto& ineighbors = instance.get_neighbors_of(i);

                const int ineighbors_end = std::min<int>(ineighbors.size(), ineighbors_begin + max_num_neighbors);

                for (int p = ineighbors_begin; p < ineighbors_end; ++p) {
                    ...
                    const int jneighbors_begin = 1;
                    const int jneighbors_end = std::min<int>(jneighbors.size(), jneighbors_begin + max_num_neighbors);
                    assert(jneighbors_end - 1 < static_cast<int>(jneighbors.size()));

                    const double cij = instance.get_cost(i, j);
                    const double cjn = instance.get_cost(j, jneighbors[jneighbors_end - 1]);
                    if (cij > cjn) {
                        insert(j, i);
                        continue;
                    }
        ...
        void set_active_percentage(std::vector<double>& percentage, std::vector<int>& vertices) {
            ...
            for (const int vertex : vertices) {
                ...
                const int num_neigbors = std::round(percentage[vertex] *
                                                    std::min<int>(max_num_neighbors, instance.get_neighbors_of(vertex).size() - 1));
