...
inline cobra::Solution routemin(const cobra::Instance &instance, cobra::Solution &best_solution, std::mt19937 &rand_engine,
    ...
    auto rvnd0 = cobra::RandomizedVariableNeighborhoodDescent</*record=*/true, /*handle_partial_solutions=*/true>(
    ...
    for (auto iter = 0; iter < max_iter; iter++) {
        ...
        for (auto selected_route : selected_routes) {
            ...
            do {
                ...
                solution.remove_vertex</*record=*/true>(selected_route, curr);
        ...
        for (auto i : removed) {
            ...
            if (best_route == -1) {
                ...
                if (r > t || solution.get_routes_num() < kmin) {
                    solution.build_one_customer_route</*record=*/true>(i);
                } else {
                    still_removed.push_back(i);
                }
            } else {
                solution.insert_vertex_before</*record=*/true>(best_route, best_where, i);
            }
        }
