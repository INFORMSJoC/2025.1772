...
public:
    ...
    // Performs a complete ruin and recreate application.
    template <bool record>
    int apply(cobra::Solution& solution, std::vector<int>& omega) {
        const int seed = ruin<record>(solution, omega);
        recreate<record>(solution);
        return seed;
    }

    // Performs a ruin application. This changes the state of the object and stores some data for a subsequent recreate.
    // Recreate should be called afterwards with the same ruined solution.
    // Note: use `apply` if you are not interested in the solution after the ruin only.
    template <bool record>
    int ruin(cobra::Solution& solution, std::vector<int>& omega) {
        removed.clear();
        routes.clear();
        ...
        for (auto n = 0; n < N; n++) {
            ...
            solution.remove_vertex<record>(route, curr);
    ...
    // Performs a recreate application. It should be called on the same solution that was previously passed to `ruin`.
    // Note: use `apply` if you are not interested in the solution after the ruin only.
    template <bool record>
    void recreate(cobra::Solution& solution) {
        ...
        for (auto customer : removed) {
            ...
            if (best_route == solution.get_end_route() || (2.0 * c_customer_depot < best_cost)) {
                solution.build_one_customer_route<record>(customer);
            } else {
                solution.insert_vertex_before<record>(best_route, best_where, customer);
            }
