...
namespace cobra {
    ...
    public:
        ...
        static std::optional<Instance> make(const std::string& filepath, int num_neighbors, int threads_num,
                                            const std::vector<std::vector<int>>& neighbors_ = std::vector<std::vector<int>>());

        // Creates an instance from the given instance data.
        static Instance make(const InstanceData& data, int num_neighbors, int threads_num,
                             const std::vector<std::vector<int>>& neighbors_ = std::vector<std::vector<int>>());
        ...
        // Returns the x coordinate of vertex `i` wrt depot.
        inline double get_x_coordinate_wrt_depot(int i) const {
            return xcoords[i] - xcoords[get_depot()];
        };

        // Returns the y coordinate of vertex `i` wrt depot.
        inline double get_y_coordinate_wrt_depot(int i) const {
            return ycoords[i] - ycoords[get_depot()];
        };
        ...
        inline const std::vector<std::vector<int>>& get_neighbors() const {
            return neighbors;
        };

    private:
        Instance(const InstanceData& data, int neighbors_num, int threads_num,
                 const std::vector<std::vector<int>>& neighbors_ = std::vector<std::vector<int>>());
