...
#include "filo2/base/BitMatrix.hpp"
#include "filo2/base/SmallFlatMap.hpp"
#include "AbstractOperator.hpp"

namespace cobra {

    // An implementation of AbstractOperator for the ejection chain local search operator.
    template <bool record, int max_relocation_nodes = 25>
    ...
    protected:
...
#ifndef NDEBUG
                        const int base_mg_index = move_index & (~1);
                        const int twin_mg_index = base_mg_index + 1;
                        const auto &base_mg = this->moves.get(base_mg_index);
                        const auto &twin_mg = this->moves.get(twin_mg_index);
                        if (base_mg.get_heap_index() != MoveGeneratorsHeap::unheaped || base_mg.is_computed_for_ejch()) {
                            assert(std::abs(base_mg.get_delta() - this->compute_cost(solution, base_mg)) < 0.01);
                        }
                        if (twin_mg.get_heap_index() != MoveGeneratorsHeap::unheaped || twin_mg.is_computed_for_ejch()) {
                            assert(std::abs(twin_mg.get_delta() - this->compute_cost(solution, twin_mg)) < 0.01);
                        }
#endif
        ...
        void post_processing(__attribute__((unused)) Solution &solution) override {
            ...
            for (size_t i = 0; i < this->moves.size(); ++i) {
                assert(this->moves.get(i).is_computed_for_ejch() == false);
            }
#endif
        }
