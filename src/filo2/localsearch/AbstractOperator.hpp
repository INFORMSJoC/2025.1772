...
namespace cobra {
    ...
    protected:
        ...
        virtual double compute_cost(const Solution& solution, const MoveGenerator& move) const = 0;
    ...
    public:
        ...
        bool apply_rough_best_improvement(Solution& solution) {
            ...
            while (index < T::heap.size()) {
                ...
                // assert(validate_heaped_moves(solution));
    ...
    private:
        ...
        inline void symmetric_update(const Solution& solution) {
            ...
            for (auto i : affected_vertices.get_elements()) {
                ...
                for (auto move_i1st_index : T::moves.get_move_generator_indices_involving_1st(i)) {
                    ...
                    const auto delta = T::compute_cost(move, icache, jcache);
                    heap_upsert(move, delta);
                    assert(std::fabs(T::compute_cost(solution, move) - move.get_delta()) < 0.01);
                }
            }
                    ...
                    for (const auto move_index : T::moves.get_move_generator_indices_involving_1st(i)) {
                        ...
                        if (vtimestamp[j] == currenttime) {
                            ...
                            if (jupji && jupij) {
                                ...
                                heap_upsert(twin_move, twin_delta);
                                assert(std::fabs(T::compute_cost(solution, twin_move) - twin_move.get_delta()) < 0.01);
                            }
                            ...
                            heap_upsert(move, delta1);
                            assert(std::fabs(T::compute_cost(solution, move) - move.get_delta()) < 0.01);

                            heap_upsert(twin_move, delta2);
                            assert(std::fabs(T::compute_cost(solution, twin_move) - twin_move.get_delta()) < 0.01);
                        }
                    }
        ...
        // Inserts or updates the heap with the given move.
        inline void heap_upsert(MoveGenerator& move, double delta) {
        ...
        bool validate_heaped_moves(const Solution& solution) {
            for (int i = 0; i < T::heap.size(); ++i) {
                const auto& move = *T::heap.spy(i);
                if (std::fabs(move.get_delta() - T::compute_cost(solution, move)) > 0.01) {
                    std::cout << "Operator: " << typeid(this).name() << "\n";
                    std::cout << "Error: delta cost mismatch!!\n";
                    std::cout << "Stored = " << move.get_delta() << "\n";
