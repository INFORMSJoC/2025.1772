...
namespace cobra {
    ...
    template <bool record, bool reverse_both_strings = false>
    ...
    protected:
        ...
        inline void execute(Solution &solution, const MoveGenerator &move, SparseIntSet &affected_vertices) override {
            ...
            affected_vertices.insert(iPrevPrevPrevPrevPrevPrev);
            affected_vertices.insert(iPrevPrevPrevPrevPrev);
            ...
            affected_vertices.insert(jNextNextNextNextNext);
            affected_vertices.insert(jNextNextNextNextNextNext);
            ...
            solution.insert_vertex_before<record>(jRoute, jNextNextNextNext, i);
            solution.insert_vertex_before<record>(jRoute, jNextNextNextNext, iPrev);
            solution.insert_vertex_before<record>(jRoute, jNextNextNextNext, iPrevPrev);

            solution.remove_vertex<record>(jRoute, jNext);
            solution.remove_vertex<record>(jRoute, jNextNext);
            solution.remove_vertex<record>(jRoute, jNextNextNext);


            if constexpr (reverse_both_strings) {
                solution.insert_vertex_before<record>(iRoute, iNext, jNextNextNext);
                solution.insert_vertex_before<record>(iRoute, iNext, jNextNext);
